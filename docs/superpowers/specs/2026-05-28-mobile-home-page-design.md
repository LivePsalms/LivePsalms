...
| 7 | MidSectionMotion render mode | **Force `'reduced'` mode on mobile** (revised during execution) | Original spec said force `'video'` mode + drop pinning. On implementation, the `'video'` JSX path requires a 500vh sticky wrapper + absolutely-positioned beats overlay — both designed for scroll-pinning. Dropping the pin would collapse that layout (all 5 beats visible at once). The existing `'reduced'` render mode already satisfies every mobile UX goal: no pin, stacked beats with poster images, IntersectionObserver fades. Mobile reuses it. The video is dropped on mobile in exchange for layout simplicity and code reuse. See `src/components/sections/MidSectionMotion.tsx`'s `initialRenderMode()` JSDoc for the in-code record. |
...
- **MidSectionMotion mobile path** in `src/components/sections/MidSectionMotion.tsx`:
  - `initialRenderMode()` extended to return `'reduced'` when viewport width is below the mobile breakpoint, regardless of WebGPU availability. The function is now exported for direct unit testing.
  - The existing reduced-motion JSX (5 stacked blocks with `/mid-section-poster.jpg` + beat copy, IntersectionObserver fades) handles the mobile render path with no JSX changes — pinned ScrollTrigger and WebGPU effects are unreachable.
